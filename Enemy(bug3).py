...
class gamescreen(abstractscreen):  # the gamescreen class
    ...
    def drawscreen(self, screen):
        ...
        self.rightwall = draw.rect(screen, self.colour,
                                   Rect(width - 20, 0, 20, height))  # drawing a rectangle (x,y,width,height)
        self.leftwall = draw.rect(screen, self.colour, Rect(0, 0, 20, height))  # drawing a rectangle (x,y,width,height)
        self.bottomwall = draw.rect(screen, self.colour, Rect(0, height - 70, width,
                                                              20))  # drawing a rectangle (x,y,width,height), at school do height - 20 at home do height - 70
        ...
        if Dungeon.playerlocationy == 0:
            self.topwall = draw.rect(screen, self.colour,
                                     Rect(0, 0, width, 20))  # drawing a rectangle (x,y,width,height)
            self.ulocked = True
        else:
            self.ulocked = False
        # print("UTrue")
        ...
        if Dungeon.playerlocationx == 0:
            self.leftwall = draw.rect(screen, self.colour,
                                      Rect(0, 0, 20, height))  # drawing a rectangle (x,y,width,height)
            self.llocked = True
        else:
            self.llocked = False
        # print("LTrue")
...
class enemyscreen(gamescreen):  # class enemyscreen - the next prototypes will add more to this screen
    ...
    def generate(self):
        ...
        enemy.velocity = random.randint(1, 2)
...
class enemy():
    ...
    def draw(self, screen):
        ...
        # draw.rect(screen,white,self.enemy_hitbox,1)

    def move(self, thing):
        self.dx = thing.x - self.x
        self.dy = thing.y - self.y
        self.dist = math.hypot(self.dx, self.dy)
        self.dx = self.dx / self.dist
        self.dy = self.dy / self.dist
        self.x = self.x + (self.dx * self.velocity)
        self.y = self.y + (self.dy * self.velocity)

    def hit(self):
        if self.enemy_hitbox.colliderect(bullet.bullet_hitbox):
            self.health = self.health - currentWeapon.damage
            currentWeapon.bullets.pop(currentWeapon.bullets.index(bullet))
        if self.health <= 0:
            enemyroom.enemylist.pop(enemyroom.enemylist.index(enemy))

    def attack(self):
        if self.enemy_hitbox.colliderect(player.player_hitbox):
...
class doors():  # class for the doors
    def __init__(self):
        ...
        self.bottomdoor = Rect((width / 2) - 100, height - 70, 200,
                               20)  # bottom door hitbox , at school do height - 20 at home do height - 70
    ...
    def drawdoor(self, screen):  # function to draw the doors
        ...
        self.bottomdoor = draw.rect(screen, black,
                                    self.bottomdoor)  # drawing a rectangle (x,y,width,height), at school do height - 20 at home do height - 70


class button():  # the button class
    def __init__(self, x, y, w, h,
                 image):  # this sets up the button class to have the following attributes which can be entered (x co-ord, y co-ord, width, height, button image)
        self.x = x
        self.y = y
        self.w = w
        self.h = h
...
class dungeon():  # the dungeon class
    def __init__(self):
        ...
        self.enemy_room = random.randint(1,
                                         2)  # Chooses a random number to give a 50% chance for a room to be an enemy room
    ...
    def generatemap(self):  # the function to generate the map
        ...
        self.playerlocationx = self.spawnpointx
        self.playerlocationy = self.spawnpointy
        self.map[self.spawnpointy][
            self.spawnpointx] = "S"  # after both loops are done it chooses a random index in the 2D array with the spawn point variables and sets it as 'S' which a spawn room
        self.spawnpointx = random.randint(0, 4)  # Chooses a random x co ordinate for the players spawn
        self.spawnpointy = random.randint(0, 4)  # Chooses a random y co ordinate for the players spawn
...
class player():  # class for the player
    ...
    def update(
            self):  # An update function that loops specific player functions in the game loop so it can be continuous whilst the game is running
    ...
    def moveright(self):  # The function to move the player right
        ...
        self.down = False
        self.up = False
        self.velocity = 4  # The speed the player is given
    ...
    def hit(self, damage):  # This function damages the player and takes in the damage as a parameter
        ...
        if self.invincible == False:  # if this value is false then the player can be damaged (VALIDATION)
            ...
            self.hit_cooldown = 300  # after the player gets hit then it resets the invincibilty cooldown
...
pistol = weapon(50, 20, 1)  # Instatiates an object for the weapon class called pistol - (damage,velocity,bulletcount)
...
# bug 3 - Random number of enemies does not happen every time enemy room is entered
# fix 3 - put ghost object in while loop, not if but while
